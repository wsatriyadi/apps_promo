...
/** admin **/
$config["email"]			= "wsatriyadi@gmail.com" ; 		//email for login
$config["password"]			= "91de1fabd050a26ca3775fc079a127c4bff35136" ; 		// sha1(password)
$config["utf8"]			= true; 		
...
/** push notification **/
$config["onesignal-appid"]			= "d9c45214-d2dd-4740-b32c-b9f1aacca3ca" ; 		//Your OneSignal AppId, available in OneSignal
$config["onesignal-appkey"]			= "ZTI0NjBjMWQtMDhmYy00MzU3LWI2MDMtMjQyMzM5NmQ2NzY3" ; 		//Your OneSignal AppKey
/** CONFIG:END **/


/** LANGGUAGE:START **/
define("LANG_PUSH_NOTIFICATION","Push Notifications");
define("LANG_FREE","Frees");
define("LANG_ID","Ids");
define("LANG_TITLE","Titles");
define("LANG_THUMBNAIL","Thumbnails");
define("LANG_LINK","Links");
define("LANG_TEXT","Texts");
/** default constant **/
...
if($_SESSION["IS_ADMIN"]==true){
	...
	$tags_html .= "</ul>" ;
	$tags_html .= "<h1 class=\"text-muted\">Apps Promo</h1>" ;
	$tags_html .= "<p>Aplikasi Promo Harian</p>" ;
	$tags_html .= "</div>" ;
	$tags_html .= "</div>" ;
	$tags_html .= "<div class=\"container-fluid\">" ;
	...
	if($_GET["table"]=="frees"){
		$tags_html .= "<li class=\"active\"><a href='?table=frees'>". @LANG_FREE."</a></li>" ;
	}else{
		$tags_html .= "<li><a href='?table=frees'>". @LANG_FREE."</a></li>" ;
	}
	...
	switch($_GET["table"]){
		case "home":
					$tags_html .= "<h4 class=\"page-title\">Apps Promo</h4>" ;
					$tags_html .= "<div class=\"row\">" ;
					$tags_html .= "<div class=\"col-md-4\">" ;
					$tags_html .= "<div class=\"panel panel-primary\" >" ;
					$tags_html .= "<div class=\"panel-heading\">" ;
					$tags_html .= "<h4  class=\"panel-title\"><span class=\"glyphicon glyphicon-list-alt\"></span> ". @LANG_FREE ."</h4>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "<div class=\"panel-body\" style=\"min-height: 150px;\">" ;
					$tags_html .= "<ul class=\"list\">" ;
					/** fetch data from free **/
					$sql_query = "SELECT * FROM `free` ORDER BY `id`  DESC LIMIT 0 , 5" ;
					...
					$tags_html .= "</ul>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "<div class=\"panel-footer text-right\"><a href=\"?table=frees\" class=\"btn btn-sm btn-primary\">".LANG_MORE."</a></div>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "</div>" ;
		break;
		// TODO: Frees
		case "frees":
					$tags_html .= "<h4>". @LANG_FREE ."</h4>" ;
				switch($_GET["action"]){
					// TODO: ---- listing
					case "list":
					$tags_html .= "<ul class=\"nav nav-tabs\">" ;
					$tags_html .= "<li class=\"active\"><a href=\"?table=frees\">".@LANG_LIST."</a></li>" ;
					$tags_html .= "<li><a href=\"?table=frees&action=add\">".@LANG_ADD."</a></li>" ;
					$tags_html .= "</ul>" ;
					$tags_html .= "<br/>" ;
					...
					$table_html .= "<th>".@LANG_THUMBNAIL."</th>" ;
					$table_html .= "<th>".@LANG_LINK."</th>" ;
					$table_html .= "<th>".@LANG_TEXT."</th>" ;
					$table_html .= "<th style=\"width:100px;\">Action</th>" ;
					$table_html .= "</tr>" ;
					$table_html .= "</thead>" ;
					$table_html .= "<tbody>" ;
					/** fetch data from mysql **/
					$sql_query = "SELECT * FROM `free`" ;
					if($result = $mysql->query($sql_query)){
						while ($data = $result->fetch_array()){
							$table_html .= "<tr>" ;
							$table_html .= "<td>" . htmlentities(stripslashes($data["title"])) . "</td>" ;
							$table_html .= "<td><img src=\"" . $data["thumbnail"] . "\" alt=\"#\" width=\"64\" height=\"64\" /></td>" ;
							$table_html .= "<td>" . htmlentities(stripslashes($data["link"])) . "</td>" ;
							$content_html = substr( strip_tags($data["text"]),0,50);
							$table_html .= "<td>" . stripslashes($content_html). "...</td>" ;
							$table_html .= "<td>" ;
							$table_html .= "<div class=\"btn-group\" >" ;
							if($config["onesignal-appkey"] !== ""){
								$table_html .= "<a target=\"_blank\" class=\"btn btn-sm btn-info\" href=\"?table=push-notification&specific=free_singles/". $data["id"]. "\"><span class=\"glyphicon glyphicon-bell\"></span></a> " ;
							}
							$table_html .= "<a class=\"btn btn-sm btn-warning\" href=\"?table=frees&action=edit&id=". $data["id"]. "\"><span class=\"glyphicon glyphicon-pencil\"></span></a> " ;
							$table_html .= "<a onclick=\"return confirm('Are you sure you want to delete ID#". $data["id"]. "')\" class=\"btn btn-sm btn-danger\" href=\"?table=frees&action=delete&id=". $data["id"]. "\"><span class=\"glyphicon glyphicon-trash\"></span></a> " ;
							$table_html .= "</div>" ;
							$table_html .= "</td>" ;
							$table_html .= "</tr>" ;
							$table_html .= "\r\n" ;
			...
			case "add":
				// TODO: ---- add
				$tags_html .= "<ul class=\"nav nav-tabs\">" ;
				$tags_html .= "<li><a href=\"?table=frees&action=list\">".@LANG_LIST."</a></li>" ;
				$tags_html .= "<li class=\"active\"><a href=\"?table=frees&action=add\">".@LANG_ADD."</a></li>" ;
				$tags_html .= "</ul>" ;
				$tags_html .= "<br/>" ;
				...
				if(isset($_POST["add"])){
					...
					if(isset($_POST["thumbnail"])){
						$data_thumbnail = addslashes($_POST["thumbnail"]);
					}
				if(isset($_FILES["thumbnail-upload"]["name"])){
					if($_FILES["thumbnail-upload"]["name"]!=""){
						$ext = pathinfo($_FILES["thumbnail-upload"]["name"],PATHINFO_EXTENSION);
						$uploadfile =  "media/image/". sha1(time()).".".$ext;
						$uploadtemp =  $_FILES["thumbnail-upload"]["tmp_name"];
						...
						if(!is_dir(dirname(__FILE__)."/media/image/")) {
							...
							if (in_array(strtolower($ext),$config["image_allowed"])){
								if(preg_match("/image/",$mimetype["mime"])){
									...
									$data_thumbnail =  $full_url ."/". $uploadfile;
								}
							}
						}
					}
					if(isset($_POST["link"])){
						$data_link = addslashes($_POST["link"]);
					}
					if(isset($_POST["text"])){
						$data_text = addslashes($_POST["text"]);
					}
	
					/** prepare save to mysql **/
					$sql_query = "INSERT INTO `free` (`title`,`thumbnail`,`link`,`text`) VALUES ('$data_title','$data_thumbnail','$data_link','$data_text')" ;
					$stmt = $mysql->prepare($sql_query);
					$stmt->execute();
					$stmt->close();
					header("Location: ?table=frees&action=list");
				}
	
	
				/** Create Form **/
				$tags_html .= '
				<form id="free" action="" method="post" enctype="multipart/form-data">
...
<div class="form-group">
	...
	<input  id="title" class="form-control input-title" data-type="heading-1" type="text" name="title" placeholder="" />
...
-->
<div class="form-group">
	<label for="thumbnail">' . @LANG_THUMBNAIL. '</label>
	<textarea id="thumbnail" class="form-control input-thumbnail"  data-type="images" name="thumbnail" ></textarea>
	<p class="help-block">Using base64 image data (small images) <input data-target="#thumbnail" type="file" data-type="image-base64" /> or file upload <input name="thumbnail-upload" type="file" data-type="image-upload" /></p>
</div>
...
-->
<div class="form-group">
	<label for="link">' . @LANG_LINK. '</label>
	<input  id="link" class="form-control input-link" data-type="link" type="text" name="link" placeholder="http://your-domain/pages" />
	<p class="help-block"></p>
</div>
...
-->
<div class="form-group">
	<label for="text">' . @LANG_TEXT. '</label>
	<textarea id="text" class="form-control input-text"  data-type="to_trusted" name="text" ></textarea>
	<p class="help-block"></p>
</div>
					<div class="form-group">
						<label for="add"></label>
		...
		case "edit":
			...
			if(isset($_GET["id"])){
				...
				if(isset($_FILES["thumbnail-upload"]["name"])){
					...
					/** update data to sql **/
					$sql_query = "UPDATE `free` SET `title` = '$data_title',`thumbnail` = '$data_thumbnail',`link` = '$data_link',`text` = '$data_text' WHERE `id`=$entry_id" ;
					$stmt = $mysql->prepare($sql_query);
					$stmt->execute();
					$stmt->close();
					header("Location: ?table=frees&action=list");
			...
			/** fetch current data **/
			$sql_query = "SELECT * FROM `free`  WHERE `id`=$entry_id LIMIT 0,1" ;
			if($result = $mysql->query($sql_query)){
				while ($data = $result->fetch_array()){
					$rows[] = $data;
				}
			...
			if(isset($rows[0]["thumbnail"])){
				$data_thumbnail = stripslashes($rows[0]["thumbnail"]) ;
			}
			if(isset($rows[0]["link"])){
				$data_link = stripslashes($rows[0]["link"]) ;
			}
			if(isset($rows[0]["text"])){
				$data_text = stripslashes($rows[0]["text"]) ;
			}
	
			/** buat form edit **/
			$tags_html .= '
<form action="" method="post" enctype="multipart/form-data" id="free">
...
<div class="form-group">
	<label for="title">' . @LANG_TITLE. '</label>
	<input  id="title" class="form-control input-title" data-type="heading-1" type="text" name="title" placeholder="" value="'.htmlentities($data_title).'" />
	<p class="help-block"></p>
</div>
...
<div class="form-group">
	<label for="thumbnail">' . @LANG_THUMBNAIL. '</label>
	<textarea id="thumbnail" class="form-control input-thumbnail"  data-type="images" name="thumbnail" >'.htmlentities($data_thumbnail).'</textarea>
	<p class="help-block">Using base64 image data (small images) <input data-target="#thumbnail" type="file" data-type="image-base64" /> or file upload <input name="thumbnail-upload" type="file" data-type="image-upload" /></p>
</div>
...
<div class="form-group">
	<label for="link">' . @LANG_LINK. '</label>
	<input  id="link" class="form-control input-link" data-type="link" type="text" name="link" placeholder="http://your-domain/pages" value="'.htmlentities($data_link).'" />
	<p class="help-block"></p>
</div>
...
<div class="form-group">
	<label for="text">' . @LANG_TEXT. '</label>
	<textarea id="text" class="form-control input-text"  data-type="to_trusted" name="text" >'.htmlentities($data_text).'</textarea>
	<p class="help-block"></p>
</div>
<div class="form-group">
	...
	<input class="btn btn-primary" type="submit" name="edit" />
			...
			// TODO: ---- delete
				...
				if(isset($_GET["id"])){
					...
					$entry_id = (int)$_GET["id"];
					/** delete item in sql **/
					$sql_query = "DELETE FROM `free` WHERE `id`=$entry_id" ;
					$stmt = $mysql->prepare($sql_query);
					$stmt->execute();
					$stmt->close();
					header("Location: ?table=frees&action=list");
		...
		$tags_html .= "<div class=\"container-fluid\">" ;
		$tags_html .= "<div class=\"row\">" ;
		$tags_html .= "<div class=\"col-md-12\"><p class=\"text-left navbar-text\">Copyright &copy; Sera Soft 2018 - All Rights Reserved</p></div>" ;
		$tags_html .= "</div>" ;
